...
- [x] **Step 6:** For every regression found during the walkthrough: fix it immediately (this is explicitly in scope per the user's request — "not new bugs introduced, new bugs identified fixed"), re-test, re-walk that specific flow.
- [x] **Step 7:** Final commit, then report: summary of what changed, the "config/blocklist cross-sync" behavior improvement noted in Task 6, any regressions found+fixed during the walkthrough, and confirmation that `pnpm typecheck && pnpm lint && pnpm test` are green.

## Completion Report (2026-09-20)

All 15 tasks done, verified, committed (16 commits total: this plan + 15 implementation commits). Final state:

- `rg -n "api\." web/src --glob '!api.ts' --glob '!queries.ts'` → zero direct `api.*` call sites outside `queries.ts` (only type-only imports, comments, the two explicit URL-builder exceptions `exportUrl`/`imageUrl`, and one intentional `queryClient.fetchQuery(...)` cache-reuse call in `PrivateStoragePanel.prepare()`).
- `pnpm typecheck` clean. `pnpm lint`: 27 warnings, identical to the pre-migration baseline (all pre-existing a11y/key noise, zero new). `pnpm test`: 1187 passed / 216 skipped / 0 failed (baseline dropped from 1190→1187 as `history-request-gate.test.ts` (2 tests) and `sheet-save-queue.test.ts` (1 test) were deleted along with the modules they tested, both subsumed by real TanStack Query v5 behavior). `pnpm build:web` succeeds.
- No actual WebSocket usage exists anywhere in this codebase — the "except websocket" carve-out in the original request had nothing to except. The one streaming call (`api.playStream`) is in scope and converted (Task 5, Pattern I: wrapped as a `useMutation`, internals unchanged — a genuine query-shaped primitive exists in v5 (`streamedQuery`) but was the wrong fit for a one-shot mutation-shaped action, confirmed against real docs rather than assumed).
- Two deliberate simplifications the migration itself enabled, once real TanStack Query behavior was confirmed against docs/source: `HistoryRequestGate` (hand-rolled stale-response guard) and `createSheetSaveQueue` (hand-rolled serial-write queue) are both deleted — their jobs are now done by query fetch-supersession and `mutation.scope`, respectively.

**Gaps found by the domain tasks' own sweeps and closed before Task 15:** `PrivateStoragePanel.prepare()`'s direct `api.stories.list()` call (Task 2's domain, missed) and `CastTab`'s sheet-lock toggle buttons' direct `api.lock(...)` call (never assigned to any task — a genuine catalog gap). Both converted; see commit `838b7d2`.

**Real bug found and fixed during the migration (not new, pre-existing, fixed as part of Pattern H):** `Illustration.tsx`'s two direct `api.saveSheet` calls (in `AppearanceEditor`/portrait-discard) previously used a closed-over `sheet` prop with no serialization against `SheetEditor`'s save queue — since `PortraitPanel`/`AppearanceEditor`/`SheetEditor` are all mounted together in `CastTab` for the same entity, a stale-prop save from one could race and clobber a concurrent edit from another. `useSaveSheetMutation`'s shared `scope` + live-cache-read fixes this for all three call sites at once (Task 14, commit `1ead9f8`).

**Local browser walkthrough (Playwright, both `serve-sqlite --memory` and `pnpm serve` against the shared dev Postgres cluster):** exercised auth/session, core state, graph/entity/debounced-search, cast (sheet save + field lock — Pattern H confirmed end-to-end with real persistence), a full streaming turn play (confirmed the `useTurnQuery`-based `lastMeta` fix from Task 5 populates the why-panel correctly), threads, causality (tick), facts, timeline, settings (style preset, blocklist add — confirmed cross-file `['config']` cache sync between `WhyPanel` and `ConfigPanels`), the setup wizard's pack-onboarding flow, and stories/worlds (a real story switch, confirmed `invalidateEverything` fully refreshes to the new story's data with zero stale bleed from the old one — the single most important correctness property in the whole plan). Zero console errors across all of it.

One crash was found and root-caused, not fixed: navigating to the library tab against the `serve-sqlite` backend threw ("Cannot read properties of undefined (reading 'length')" on `w.sources.length`). Traced to the raw `GET /api/worlds` response from that specific backend mode entirely lacking the `sources`/`edgeCount`/`reading`/`visibility`/`role` fields `WorldSummary` declares — it returns a legacy pre-multi-world shape (`bytes`, `current`) instead. The crashing JSX line is untouched by any of the 15 tasks, so this reproduces identically against the pre-migration code on the same backend — a pre-existing `serve-sqlite`/frontend version-skew issue, not a migration regression. Confirmed clean against `pnpm serve` (Postgres) instead, which is what the app's own staleness banner recommends.

**Not exercised in the browser** (both require a signed-in user, which this loopback dev environment has no configured auth provider for): the encryption enroll/unlock/lock buttons, and the claim-a-story flow succeeded in reaching the server and failing gracefully with the correct business-rule error ("sign in first: there is no owner to claim these for") rather than crashing — confirming the mutation and its error handling are wired correctly even without being able to reach the success path locally. Both domains (Tasks 2 and 11's claim/unowned pieces) were verified via careful line-by-line diff review instead (documented inline in this conversation), including a specific check of the zero-grants unlock edge case.
