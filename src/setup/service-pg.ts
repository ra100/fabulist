...
import { installPack } from '../packs/apply-pg.ts';
...
export interface SetupServiceOptions {
   ...
   * already be inside a transaction and cannot begin another.
   */
  db: Db;
  providers: Registry;
...
export class SetupService {
  ...
  private async runResumablePassB(
    ...
    for (const candidate of pending) {
      ...
      const entity = page ? await world.graph.resolveName(candidate.title) : undefined;
      ...
      if (out.failed) {
        ...
        await world.db.query(
          `UPDATE ingest_pages SET passb_status = 'failed' WHERE world_id = $1 AND wiki = $2 AND page_id = $3`,
          [this.requireWorldId(world), wikiName, page.pageId],
        );
        failed++;
      ...
      await applyPassB(world, spec, candidate.title, entity, out, counters);
      await world.db.query(
        `UPDATE ingest_pages SET passb_status = 'done' WHERE world_id = $1 AND wiki = $2 AND page_id = $3`,
        [this.requireWorldId(world), wikiName, page.pageId],
      );
      done++;
      handle.count(done + failed, pending.length);
  ...
  async ingestHealth(): Promise<IngestHealth> {
    ...
    const { rows } = await world.db.query<{ done: string | null; failed: string | null; pending: string | null }>(
      `SELECT
       ...
       FROM ingest_pages WHERE world_id = $1 AND wiki = $2`,
      [this.requireWorldId(world), context.wikiName],
    );
    const counts = {
      done: rows[0]?.done === null || rows[0]?.done === undefined ? 0 : Number(rows[0].done),
      failed: rows[0]?.failed === null || rows[0]?.failed === undefined ? 0 : Number(rows[0].failed),
      pending: rows[0]?.pending === null || rows[0]?.pending === undefined ? 0 : Number(rows[0].pending),
    };
  ...
  async usePack(
    ...
    const scenarioWorld = await world.withStory(chosen.storyId);
    return {
      storyId: chosen.storyId,
      scenarioId: chosen.id,
      title: pack.title,
      playerCharacterId: chosen.playerCharacterId,
      opening: chosen.opening || (await proposeOpening(scenarioWorld)),
   ...
   */
  /**
   * Wipes one story, leaving canon and every other story alone.
   *
   * This is half of what the SQLite `reset()` did, and the half a *player* wants:
   * "start this story over". The other half — rebuilding canon — is
   * `rebuildCanon` below, and separating them is the whole point of the
   * user/system split.
   *
   * One DELETE. Every story-scoped table declares
   * `REFERENCES stories(id) ON DELETE CASCADE`, so the database does the sweep
   * that the SQLite version did by hand from a 17-table list — the list that
   * silently omitted `illustrations` (found when the integrity checker turned up
   * three portraits still pointing at `char:brother-anselm` after `entities` had
   * been emptied) and then `stories` itself. A table added later is covered
   * automatically.
   *
  ...
  /**
   * Empties a canon world so the wizard can rebuild it, leaving every story alone.
   *
   * The system half of the old `reset()`. Under SQLite this was impossible: canon
   * and stories lived in one file and the only whole-world rebuild deleted both,
   * which is why "refresh a world" did not exist as a feature. Here the play role
   * cannot even reach these tables — this runs as `fabulist_ingest`.
   *
   * Stories that read the world keep their chronicle and their prose. They will
   * reference canon ids that no longer resolve until the world is re-ingested,
   * which `checkIntegrity` reports honestly rather than hiding; that is the
   * correct behaviour for "the source material is being rebuilt underneath you",
  ...
  async rebuildCanon(worldId?: number): Promise<{ worldId: number; entities: number; edges: number }> {
    ...
    return this.db.tx(async (client) => {
      const before = await client.query<{ entities: string; edges: string }>(
        `SELECT
           (SELECT count(*) FROM canon_entities WHERE world_id = $1) entities,
           (SELECT count(*) FROM canon_edges WHERE world_id = $1) edges`,
        [target],
      );
      // Ordered so nothing references a row that is already gone. `ingest_pages`
      // goes too: keeping it would make the next ingest think every page was
      // already read and skip the expensive Pass B it needs to redo.
      for (const table of ['canon_edges', 'canon_sheets', 'canon_entities', 'ingest_pages', 'world_meta']) {
        await client.query(`DELETE FROM ${table} WHERE world_id = $1`, [target]);
      }
      await client.query(
        `UPDATE worlds SET title = '', ingest_context = '{}'::jsonb, revision_watermark = '', last_refreshed_at = NULL
           WHERE id = $1`,
        [target],
      );
      await client.query(`DELETE FROM world_sources WHERE world_id = $1`, [target]);
      return {
        worldId: target,
        entities: Number(before.rows[0]?.entities ?? 0),
        edges: Number(before.rows[0]?.edges ?? 0),
      };
    });
  }

  /** The canon world this service writes to; thrown rather than guessed. */
  private requireWorldId(world: World): number {
    const id = world.sources[0]?.worldId;
    if (id === undefined) {
