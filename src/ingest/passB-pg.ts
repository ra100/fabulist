...
export class LlmPassBExtractor implements PassBExtractor {
  ...
  async extract(page: WikiPage, entity: Entity): Promise<PassBOutput> {
    ...
    const known = await this.knownNames(entity);
    const existing = await this.existingFacts(entity);
    ...
    return await this.validate(raw, page, entity);
  }
  ...
  private async knownNames(entity: Entity): Promise<string[]> {
    const names = new Set<string>();
    // Neighbours and their names in two queries rather than one per neighbour.
    const [neighbours, salient] = await Promise.all([
      this.world.graph.edgesFrom(entity.id),
      this.world.graph.list({ limit: 120 }),
    ]);
    const neighbourNames = await this.world.graph.getMany(neighbours.map((e) => e.object));
    for (const n of neighbourNames.values()) names.add(n.name);
  ...
  private async existingFacts(entity: Entity): Promise<string> {
    const lines: string[] = [];
    const edges = (await this.world.graph.edgesFrom(entity.id)).slice(0, 20);
    const names = await this.world.graph.getMany(edges.map((e) => e.object));
    for (const edge of edges) {
      if (edge.predicate === 'MENTIONS') continue;
      lines.push(`${entity.name} ${edge.predicate} ${names.get(edge.object)?.name ?? edge.object}`);
    }
   ...
   */
  private async validate(raw: unknown, page: WikiPage, entity: Entity): Promise<PassBOutput> {
    ...
    for (const r of asArray(o.relations)) {
      ...
      const target = await this.world.graph.resolveName(objectName);
