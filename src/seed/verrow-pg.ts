...
export async function seedWorld(world: World, opts: SeedOptions = {}): Promise<void> {
  ...
  for (const [subject, predicate, object, weight] of EDGES) {
    await world.graph.assertEdge({ subject, predicate, object, weight }, 0, 'canon', 'authored');
  }
  ...
  await world.cast.put({
    ...
    locks: [],
    // Written false on the canon baseline: `is_player` lives on chronicle only,
    // because the protagonist belongs to a playthrough rather than to the source
    // material. The chronicle flag is set immediately below.
    isPlayer: false,
  }, 'canon');
  ...
  for (const [from, to, trust, affection, respect, note] of rels) {
    await world.cast.adjustRelationship(from, to, { trust, affection, respect, note });
  }
  ...
  const routeFact = await world.chronicle.addFact('the Order moves people over the pass and records them in the psalter', 0);
  await world.chronicle.setKnowledge(routeFact.id, 'char:brother-anselm', 'knows', 0);
  await world.chronicle.setKnowledge(routeFact.id, 'char:sister-oria', 'knows', 0);
  await world.chronicle.setKnowledge(routeFact.id, 'char:hela-vask', 'knows', 0);
  await world.chronicle.setKnowledge(routeFact.id, 'char:captain-sered', 'suspects', 0);
  await world.chronicle.setKnowledge(routeFact.id, 'char:sergeant-doff', 'suspects', 0);

  const seredFact = await world.chronicle.addFact('Sered has known about the route for a year and has not reported it', 0);
  await world.chronicle.setKnowledge(seredFact.id, 'char:captain-sered', 'knows', 0);

  const priorFact = await world.chronicle.addFact('the last prior was killed by the garrison, not by fever', 0);
  await world.chronicle.setKnowledge(priorFact.id, 'char:prior-galt', 'knows', 0);
  await world.chronicle.setKnowledge(priorFact.id, 'char:captain-sered', 'knows', 0);
  await world.chronicle.setKnowledge(priorFact.id, 'char:brother-anselm', 'suspects', 0);
  ...
  await world.chronicle.upsertScene(1, {
  ...
  const playerId = opts.playerCharacterId ?? 'char:brother-anselm';
  ...
  // Flag the protagonist in chronicle, over the canon baseline written above.
  //
  // `is_player` exists only on `chron_sheets`: who the protagonist is belongs to a
  // playthrough, not to the source material, and two stories reading this world
  // have different players. Writing it on canon drops it silently, which is what
  // made `cast.player()` return nothing for a seeded world until a test caught it.
  const playerBaseline = await world.cast.get(playerId);
  if (playerBaseline) await world.cast.put({ ...playerBaseline, isPlayer: true }, 'chronicle');

  // A style anchor from the outset, so the narrator has a texture to match
  // before the player has liked anything.
  await world.chronicle.addAnchor(
