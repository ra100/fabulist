...
function buildServer(ctx: McpToolContext, resourceUrl: string): McpServer {
  ...
  server.registerTool(
    ...
    async ({ scene, chapter, turnId, mode }) => toolResult(rollbackTool(ctx, { scene, chapter, turnId, mode })),
  );
  ...
  server.registerTool(
    ...
    async ({ turnId }) => toolResult(splitSceneTool(ctx, { turnId })),
  );
  ...
  server.registerTool(
    ...
    async ({ id, prose, stateMode, world }) =>
      toolResult(replaceTurnProseTool(ctx, { id, prose, stateMode, world })),
  );
  ...
  server.registerTool(
    ...
    async ({ text, knownBy, suspectedBy }) => toolResult(recordFactTool(ctx, { text, knownBy, suspectedBy })),
  );
  ...
  server.registerTool(
    ...
    async ({ title, stakes, parties, tension, resolutions }) =>
      toolResult(openThreadTool(ctx, { title, stakes, parties, tension, resolutions })),
  );
  ...
  server.registerTool(
    ...
    async ({ causeEventId, actorId, action, trigger, visibility, significance }) =>
      toolResult(addConsequenceTool(ctx, { causeEventId, actorId, action, trigger, visibility, significance })),
  );
