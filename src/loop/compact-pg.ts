...
export class Compactor {
  ...
  async summariseScene(
    ...
    // Both batched: the roster was one query per participant, and a long scene
    // names many. `getMany` and one `events` read replace N+1 round trips.
    ...
    const events = (suppliedEvents ?? await world.chronicle.events({ limit: Number.MAX_SAFE_INTEGER }))
    ...
    await world.chronicle.upsertScene(scene, {
      summary,
  ...
  async summariseChapter(world: World, chapter: number, force = false): Promise<string | null> {
    ...
    const scenes = (await world.chronicle.scenes()).filter((s) => s.chapter === chapter && s.summary);
    ...
    if (!summary) return null;
    await world.chronicle.upsertChapter(chapter, {
      summary,
      title: typeof parsed.title === 'string' ? parsed.title.slice(0, 90) : '',
    });
    return summary;
  ...
  async onSceneClosed(world: World, closedScene: number): Promise<CompactionResult> {
    ...
    // The SQLite version folded the WAL back in here, a scene boundary being the
    // one moment in play that is already a pause. Postgres manages its own WAL and
    // checkpointing, so there is nothing for the application to do — the hook is
    // gone rather than stubbed, because a no-op named `checkpoint` would imply the
  ...
  async backfill(world: World, currentScene: number): Promise<CompactionResult> {
    ...
    for (const event of await world.chronicle.events({ limit: Number.MAX_SAFE_INTEGER })) {
    ...
    for (const scene of [...scenesWithTurns].sort((a, b) => a - b)) {
      ...
      const summary = await this.summariseScene(
        world, scene, false, layout, identity ? eventsByIdentity.get(identity) ?? [] : [],
      );
