 ...
 */
export function exportMarkdown(world: World, opts: ExportOptions = {}): string {
 ...
 */
export function exportPlainText(world: World, opts: ExportOptions = {}): string {
  const md = exportMarkdown(world, opts);
  return md
    .split('\n')
    .map((line) => {
      const h1 = line.match(/^# (.*)$/);
