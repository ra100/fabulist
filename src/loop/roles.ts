/**
 * Role prompts and invocations. See DESIGN.md §1 (three-role split) and §4.
 *
 * The Narrator never invents world facts; it renders what the Referee and
 * Director agreed on. Keeping that boundary in the prompts is most of what makes
 * the state layer trustworthy.
 */
import type {
  CoherenceDistance,
  Delta,
...
export async function classify(deps: RoleDeps, rawInput: string, actorId: EntityId): Promise<Intent> {
  ...
  const targetIds = targetNames
    .map((n) => deps.world.graph.resolveName(n)?.id)
...
export async function integrity(
  ...
  const frame = buildIntegrityFrame(ctx, actorId);
  ...
  const interrupt = shouldInterrupt(distance, strictness)
    ? buildInterrupt(deps, actorId, violatedVows, distance)
    : null;
...
function buildInterrupt(
  ...
  const world = deps.world;
  const entity = world.graph.get(actorId);
...
export async function direct(deps: RoleDeps, rawInput: string): Promise<DirectorPlan> {
  ...
  const frame = buildDirectorFrame(ctx);
...
export async function extract(
  ...
  const frame = buildExtractFrame(ctx, prose);
