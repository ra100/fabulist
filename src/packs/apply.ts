...
function installScenario(
  ...
  const story = createStory(world.db, { title: scenario.title });
  const w = world.withStory(story.id);
  ...
  for (const [entityId, tier] of Object.entries(focus) as Array<[string, SalienceTier]>) {
    ...
    w.graph.setSalience(entityId, SALIENCE[tier]);
  }
  ...
  w.graph.setSalience(scenario.playerCharacterId, SALIENCE.focal);
  ...
  for (const c of scenario.conditions ?? []) {
    ...
    const existing = w.cast.get(entityId);
    w.cast.put({
      entityId,
  ...
  if (!(scenario.conditions ?? []).some((c) => c.entityId === scenario.playerCharacterId)) {
    const existing = w.cast.get(scenario.playerCharacterId);
    w.cast.put({
  ...
  const playerSheet = w.cast.get(scenario.playerCharacterId);
  ...
  for (const r of scenario.relationships ?? []) {
    ...
    w.cast.adjustRelationship(r.from, r.to, {
      trust: r.trust ?? 0,
  ...
  for (const f of scenario.facts ?? []) {
    const fact = w.chronicle.addFact(f.text, 0);
    for (const [entityId, level] of knowledgeLevels(f)) {
      ...
      w.chronicle.setKnowledge(fact.id, entityId, level, 0);
    }
  }
