#!/usr/bin/env bash
# Runs on the VPS with a real action argument. GitHub Actions sends the bare
# commands `upload-env` and `deploy`; the root-owned forced-command dispatcher
# in deploy/ssh-command.sh validates SSH_ORIGINAL_COMMAND and invokes this file
# with the corresponding argv. Keeping SSH command parsing out of this script
# also makes direct operator use (`./deploy.sh deploy`) unchanged.
set -euo pipefail
cd "$(dirname "$0")"
...
load_app_env() {
  ...
  while IFS='=' read -r name value || [ -n "$name" ]; do
    case "$name" in
      ...
      FABULIST_PG|POSTGRES_PASSWORD|COMPOSE_PROFILES|FABULIST_DB_ROLES|FABULIST_SECRETS_KEY)
        export "$name=$value"
        ;;
      *)
        echo "deploy.sh: app.env contains an unsupported variable name" >&2
        exit 1
        ;;
    esac
  done < app.env
  ...
  upload-env)
    ...
    while IFS='=' read -r name _ || [ -n "$name" ]; do
      case "$name" in
        ...
        AUTH_REQUIRE_LOGIN|AUTH_PUBLIC_ORIGIN|AUTH_ADMIN_EMAILS|\
        WORKOS_API_KEY|WORKOS_CLIENT_ID|WORKOS_COOKIE_PASSWORD|\
        FABULIST_PG|POSTGRES_PASSWORD|COMPOSE_PROFILES|FABULIST_IMAGE|FABULIST_DB_ROLES|FABULIST_SECRETS_KEY)
          ;;
        *)
          rm -f app.env.tmp
          echo "deploy.sh: uploaded app.env contains an unsupported variable name" >&2
          exit 1
          ;;
      esac
    done < app.env.tmp
